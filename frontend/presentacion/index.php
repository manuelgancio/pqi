...
/* Controlo errores */
	if(isset($_GET['err'])){
		if ($_GET['err'] == 'fecha'){
			?>
			<script>
			error('No hay art�culos en esa fecha.','7000');
			</script>
			<?php	
		}elseif ($_GET['err'] == 'regOk'){
			?>
			<script>
			aviso('Registro correcto! Ya puedes iniciar sesi�n.','7000');
			</script>
			<?php	
		}elseif($_GET['err'] == 'susOk'){
			?>
			<script>
			aviso('La suscripción se realizo con �xito.','7000');
			</script>
			<?php	
		}elseif($_GET['err'] == 'susC'){
			?>
			<script>
			aviso('Se cancelo la suscripci�n.','7000');
			</script>
			<?php	
		}elseif($_GET['err'] == 'loginF'){
			?>
			<script>
			error('Usuario o contrase�a incorrecta.','7000');
			</script>
			<?php	
